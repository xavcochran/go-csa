...
func filter(filepathIn, filepathOut string, threads int) {
	...
	height := bounds.Dy()
	width := bounds.Dx()
	filterChan := make([]chan [][]uint8, threads)
	for i := 0; i < threads; i++ {
		filterChan[i] = make(chan [][]uint8)
	}
	...
	if threads == 1 {
		...
		for i := 0; i < threads; i++ {
			go worker(i*height/threads, (i+1)*height/threads, 0, width, immutableData, filterChan[i])
		}
		for i := 0; i < threads; i++ {
			newPixelData = append(newPixelData, <-filterChan[i]...)
		}
	}
